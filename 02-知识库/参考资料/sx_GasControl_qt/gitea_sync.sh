...
echo -e "${YELLOW}      首次操作仓库时需要输入一次用户名和密码，之后无需重复输入。${NC}"

# 配置远程仓库
echo -e "\n${BLUE}配置远程Gitea仓库...${NC}"
echo -e "${YELLOW}添加Gitea远程仓库：$TEAM_REPO_URL${NC}"
...
echo -e "\n${GREEN}Gitea仓库配置完成！${NC}"
echo -e "\n${BLUE}以下是使用指南：${NC}"
echo -e "\n${YELLOW}1. 首次推送代码到Gitea仓库：${NC}"
echo "   git push -u gitea master"
echo -e "\n${YELLOW}2. 日常开发后推送到Gitea仓库：${NC}"
echo "   git push gitea master"
echo -e "\n${YELLOW}3. 再次运行此脚本可进行仓库同步：${NC}"
echo "   bash gitea_sync.sh"
...
if [ $? -ne 0 ]; then
echo "警告：推送到Gitea仓库失败！"
echo "注意：可能需要先在Gitea创建同名仓库，或者检查网络连接和权限设置。"
fi

echo -e "\n${GREEN}Gitea仓库同步完成！${NC}"
fi