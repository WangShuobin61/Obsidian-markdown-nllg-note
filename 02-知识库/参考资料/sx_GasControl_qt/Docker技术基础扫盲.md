...
- 你的现状 ：你在 Ubuntu 22.04 上 make install 出来的程序，直接拷贝到 UOS / 银河麒麟 上是跑不起来的，因为 UOS / 银河麒麟 缺少这道菜需要的“餐具”（依赖库）。
...
- 局限性 ： 它非常依赖底层系统 （glibc 版本）。
  - 这就是你当前遇到的最大痛点：你在 Ubuntu 22.04（glibc 2.35）上用 linuxdeployqt 打包的 AppImage，拿到 UOS / 银河麒麟 V20（基于 Debian 10，glibc 2.28）上运行， 大概率会报错 version 'GLIBC_2.29' not found 。
  - 因为外卖盒（打包）虽然带了餐具（Qt 库），但它假设客人桌子（操作系统内核/glibc）是新的，结果 UOS / 银河麒麟 的桌子太旧了，放不下。
### 3. Docker 打包 —— “移动餐车”
- 是什么 ：这是一个 虚拟化交付 。
...
- 优势 ： 彻底屏蔽底层差异 。
  - 不管你的宿主机是 UOS / 银河麒麟、CentOS 还是 Fedora，只要装了 Docker 引擎，你的“移动餐车”就能跑。
  - 容器内部依然是 Ubuntu 22.04 环境，所以你编译出来的程序能完美运行，根本感觉不到外面其实是 UOS / 银河麒麟。
### 总结对比表
方式 产物 能在 UOS / 银河麒麟 上跑吗？ 为什么？ 典型用途 CMake Install 一堆分散的文件 不能 缺库，缺环境，版本不兼容 本地开发调试 linuxdeployqt AppImage 单文件 很难 虽带了 Qt 库，但被底层的 glibc 版本卡死 同架构分发 (如 Ubuntu 发给 Ubuntu) Docker Docker 镜像 完美 自带完整 OS 环境，不依赖 UOS / 银河麒麟 的库 跨系统/跨版本部署 (工业标准)

### 你的最佳实践路线
针对你的项目（Qt 6 + ROS 2 Humble -> 部署到 UOS / 银河麒麟）：

1. 放弃 linuxdeployqt ：不要试图在 Ubuntu 22.04 上打包 AppImage 给 UOS / 银河麒麟 用，glibc 版本回退问题会让你崩溃。
2. 拥抱 Docker ：
   - 编写 Dockerfile。
   - 在 Dockerfile 里写上 cmake 和 make install 的命令。
   - 这样构建出来的 Docker 镜像，里面就包含了已经编译好、安装好的程序。
   - 把这个镜像丢给 UOS / 银河麒麟，它就能跑了。
   所以， Docker 打包 = ( 基础镜像 Ubuntu 22.04 ) + ( 在里面执行 CMake 构建 ) + ( 设置启动入口 ) 。它包含了前者的功能，但提供了更高级的封装。

 利用 Docker 屏蔽底层操作系统的差异 。

UOS / 银河麒麟 V20 基于 Debian 10，内核和 glibc 较老，直接运行 ROS 2 Humble（基于 Ubuntu 22.04）和最新的 Qt 6.10 非常困难。Docker 是目前最完美的解决方案。
...
## Q3：在统信 UOS / 银河麒麟 V20 上安装 Docker

结论：完全可以安装。 UOS / 银河麒麟 V20 是标准的 Linux 发行版（Debian 系），Docker 的支持非常好。

详细步骤：

1. 开启开发者模式（关键第一步）

   - 在 UOS / 银河麒麟 系统中，打开“控制中心” -> “通用” -> “开发者模式”。
   - 点击“进入开发者模式”，通常需要登录 UOS / 银河麒麟 账号并重启。
   - 原因：默认情况下 UOS / 银河麒麟 禁止安装非应用商店的软件和获取 root 权限。
...
2. 准备环境与添加源 打开终端（Terminal），执行以下命令：
   ...
   # 3. 添加软件源
   # 注意：这里必须强制指定 "buster"，因为 UOS / 银河麒麟 V20 对应 Debian 10 
   (Buster)
...
你的理解是正确的： “在 Ubuntu 22.04 开发，在 Docker 里打包，在 UOS / 银河麒麟 上通过 Docker 运行” 。
...
- 部署环境 (Target Host) : 统信 UOS / 银河麒麟 V20 机器（已安装 Docker）。
...
阶段三：部署 (Deployment)
拿着这个 robot_system_v1.0.tar 文件（几 GB 大小）去 UOS / 银河麒麟 机器。
...
3. 启动上位机 (Qt 界面)： 这是最麻烦的一步，因为 Docker 本身没有屏幕，它需要把画面“投射”到 UOS / 银河麒麟 的桌面上

**总结核心要点:**

1. 网络互通 ：使用 --net=host 参数。这样 Docker 里的上位机和下位机就像运行在同一台机器的两个进程一样，ROS 2 的 DDS 通信会自动发现对方，完全不需要配置 IP。
2. 图形界面 ：通过 -v /tmp/.X11-unix:/tmp/.X11-unix 和 -e DISPLAY 把 UOS / 银河麒麟 的显示接口借给 Docker 用。
3. 硬件访问 ：使用 --privileged 参数，确保下位机能访问 USB 转串口、CAN 卡或网口。
4. 开发分离 ：你依然在最舒服的 Ubuntu 22.04 上写代码，UOS / 银河麒麟 只是一个负责运行 Docker 的“载体”。
